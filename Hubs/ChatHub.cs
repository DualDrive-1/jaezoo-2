...
namespace JaeZoo.Server.Hubs
{
    ...
    public class ChatHub : Hub
    {
        ...
        /// <summary>Список userId, которые подключены и не скрывают присутствие.</summary>
        ...
        // --------------------------------------------------
        // Typing indicator (краткоживущие события)
        // --------------------------------------------------

        /// <summary>Клиент сообщает, что начал печатать.</summary>
        public async Task TypingStarted(Guid peerId)
        {
            var me = MeId;
            if (!await AreFriends(me, peerId))
                throw new HubException("Вы не друзья.");

            await Clients.User(peerId.ToString()).SendAsync("TypingStarted", me.ToString());
        }

        /// <summary>Клиент сообщает, что перестал печатать.</summary>
        public async Task TypingStopped(Guid peerId)
        {
            var me = MeId;
            if (!await AreFriends(me, peerId))
                throw new HubException("Вы не друзья.");

            await Clients.User(peerId.ToString()).SendAsync("TypingStopped", me.ToString());
        }

        /// <summary>«Пульс» во время набора — продлевает индикатор.</summary>
        public async Task Typing(Guid peerId)
        {
            var me = MeId;
            if (!await AreFriends(me, peerId))
                throw new HubException("Вы не друзья.");

            await Clients.User(peerId.ToString()).SendAsync("Typing", me.ToString());
        }

        /// <summary>Универсальный метод (true=печатает, false=остановил).</summary>
        public async Task TypingChanged(Guid peerId, bool isTyping)
        {
            var me = MeId;
            if (!await AreFriends(me, peerId))
                throw new HubException("Вы не друзья.");

            await Clients.User(peerId.ToString()).SendAsync("TypingChanged", me.ToString(), isTyping);
        }
    }
}
